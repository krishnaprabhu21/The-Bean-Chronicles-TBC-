...
const DARK_MAP_STYLES = [
  ...
  { featureType: 'administrative', elementType: 'geometry', stylers: [{ color: '#1E3020' }] },
  { featureType: 'administrative.country', elementType: 'labels.text.fill', stylers: [{ color: '#D4A853' }] },
  { featureType: 'administrative.locality', elementType: 'labels.text.fill', stylers: [{ color: '#F5F0EB' }] },
  { featureType: 'poi', stylers: [{ visibility: 'off' }] },
  { featureType: 'road', elementType: 'geometry', stylers: [{ color: '#1C2B14' }] },
  { featureType: 'road', elementType: 'geometry.stroke', stylers: [{ color: '#0D1810' }] },
  { featureType: 'road', elementType: 'labels.text.fill', stylers: [{ color: '#3A5A30' }] },
  { featureType: 'road.highway', elementType: 'geometry', stylers: [{ color: '#1E3020' }] },
  { featureType: 'road.highway', elementType: 'geometry.stroke', stylers: [{ color: '#1C2B14' }] },
  { featureType: 'transit', stylers: [{ visibility: 'off' }] },
  { featureType: 'water', elementType: 'geometry', stylers: [{ color: '#0A0605' }] },
  { featureType: 'water', elementType: 'labels.text.fill', stylers: [{ color: '#1A3018' }] },
]
...
function FlagImage({ countryCode, name, size = 'md', accentColor, selected }) {
  const sizes = { sm: { img: 'w-10 h-7', w: 40 }, md: { img: 'w-14 h-[38px]', w: 80 }, lg: { img: 'w-20 h-14', w: 160 } }
  const s = sizes[size]
  return (
    <img
      ...
      style={{ boxShadow: selected ? `0 3px 14px ${accentColor}60` : '0 2px 6px rgba(0,0,0,0.5)' }}
    />
  )
}
...
function CountryCard({ country, selected, onClick }) {
  return (
    ...
    >
      ...
      <FlagImage countryCode={country.countryCode} name={country.name} size="sm" accentColor={country.accentColor} selected={selected} />
      <span className="text-xs sm:text-sm font-semibold leading-snug" style={{ color: selected ? '#D4A853' : '#F5F0EB' }}>
        {country.name}
      </span>
      <span className="text-[10px] uppercase tracking-widest" style={{ color: '#8B5E3C' }}>
        {country.continent}
      </span>
    </motion.button>
  )
...
function MapNoKey() {
  return (
    <div
      ...
      style={{ background: '#1C2B14', border: '1px solid rgba(80,120,60,0.25)' }}
    >
      <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#D4A853" strokeWidth="1.5">
        <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" />
        <circle cx="12" cy="10" r="3" />
      </svg>
      <div className="text-center px-6">
        <p className="text-cream font-semibold mb-1">Google Maps API key required</p>
...
function CoffeeMap({ country }) {
  ...
  if (!apiKey || apiKey === 'your_google_maps_api_key_here') return <MapNoKey />
    ...
    >
      ...
      {country.regions.map((region) => (
        <AdvancedMarker
          ...
          onClick={() => setActiveRegion((prev) => (prev?.name === region.name ? null : region))}
        >
          <div
            style={{
              width: 20,
      ...
      >
        ...
        >
          ...
          <div
            ...
            style={{ borderBottom: '1px solid rgba(80,120,60,0.14)', background: 'rgba(13,24,16,0.85)' }}
          >
            ...
            <span
              ...
              style={{ color: 'rgba(201,168,76,0.55)', fontFamily: 'Space Mono, monospace' }}
            >
              Growing Regions
            </span>
          </div>
          ...
          {/* Map */}
          <div className="flex-1 min-h-[280px] sm:min-h-0" style={{ height: '100%' }}>
          ...
          <div
            ...
            style={{ borderTop: '1px solid rgba(80,120,60,0.14)', background: 'rgba(13,24,16,0.85)' }}
          >
            {country.regions.map((r) => (
              <span
                key={r.name}
                className="flex items-center gap-2 px-3 py-1.5 text-[9px]"
                style={{ border: '1px solid rgba(80,120,60,0.3)', color: 'rgba(232,223,208,0.7)', background: 'rgba(22,34,16,0.8)' }}
              >
                <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: country.accentColor }} />
                {r.name}
              </span>
            ))}
          </div>
        ...
        >
          ...
          <div
            ...
            style={{ background: 'rgba(22,34,16,0.96)', borderBottom: '1px solid rgba(80,120,60,0.13)', backdropFilter: 'blur(6px)' }}
          >
            <span
              className="text-[9px] uppercase tracking-[0.22em]"
            ...
            <button
              ...
              style={{ color: '#E8DFD0', fontFamily: 'Space Mono, monospace', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.2em' }}
            >
              Close
          ...
          <div className="px-8 py-9 sm:px-10 sm:py-10 flex flex-col">
            ...
            <p className="font-elegant italic text-base sm:text-lg mb-9 ml-[3.5rem]" style={{ color: 'rgba(232,223,208,0.48)' }}>
            ...
            <div
              ...
              style={{ borderTop: '1px solid rgba(80,120,60,0.15)', borderBottom: '1px solid rgba(80,120,60,0.15)' }}
            >
              <div>
                <p className="text-[9px] uppercase tracking-[0.2em] mb-2" style={{ color: 'rgba(232,223,208,0.32)', fontFamily: 'Space Mono, monospace' }}>
                  Elevation
                </p>
                <p className="text-sm font-medium" style={{ color: country.accentColor }}>{country.elevation}</p>
              </div>
              <div>
                <p className="text-[9px] uppercase tracking-[0.2em] mb-2" style={{ color: 'rgba(232,223,208,0.32)', fontFamily: 'Space Mono, monospace' }}>
                  Annual Output
                </p>
                <p className="text-sm font-medium" style={{ color: country.accentColor }}>{country.annualProduction}</p>
              </div>
            </div>

            {/* History */}
            <div className="mb-9">
              <p className="text-[9px] uppercase tracking-[0.24em] mb-5" style={{ color: 'rgba(201,168,76,0.42)', fontFamily: 'Space Mono, monospace' }}>
                — History
              </p>
              <p className="text-parchment/72 text-sm sm:text-base leading-[2.05]">{country.history}</p>
            </div>
            ...
            {country.globalStanding && (
              <div className="mb-9">
                ...
                <p className="text-parchment/72 text-sm sm:text-base leading-[2.05]">{country.globalStanding}</p>
              </div>
            )}
      ...
      <section className="w-full max-w-[1600px] mx-auto px-4 sm:px-8 xl:px-16 py-20">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-8 gap-3 sm:gap-4">
          {coffeeOrigins.map((country, i) => (
            ...
            >
              <CountryCard country={country} selected={selectedId === country.id} onClick={setSelectedId} />
            </motion.div>
          ))}
        </div>
        ...
        <motion.p
          ...
          style={{ color: 'rgba(201,168,76,0.28)', fontFamily: 'Space Mono, monospace' }}
        >
